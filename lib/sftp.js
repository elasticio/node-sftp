...
(function() {
    ...
    this.mkdir = function(path, mode, callback) {
        ...
        this.exec("mkdir", "mkdir " + path, function(lines) {
            ...
            if (err)
                return callback(err);
            _self.chmod(path, mode, callback);
        });
    };
    ...
    /**
     *  Uses sftp get to output a file to a specified destination
     *  @param params {Object} An object containing the files
     *  @param params.filename {String} the remote filename to retrieve
     *  @param params.dest {String} the output destination
     */
    this.streamFile = function(params, callback) {
      if (!params.filename){
        return callback("No filename specified");
      }

      var temp = params.dest || Util.DEFAULT_TMPDIR;
      temp = Path.join(temp, params.filename);
      this.exec("readFile", "get " + '"' + params.filename + '"' + " " + '"' + temp + '"', function(lines) {
     ...
     * gets two arguments (err, files) where files is an array of the objctects.
     * Each object has a path and a sftp.Stats property.
     *
     * @param {String}        path
     * @param {Function}      callback
     * @type  {void}
     */
    this.dirStats = function(path, callback) {
        this.exec("ls", "ls -lan "+path, function(lines) {
            var err = isError(lines);
            ...
            var res = parseListing(lines);

            var result = res.map(function(line){
                return {
                    path : line.path,
                    stat:new Stat(line)
                };
    ...
    function parseReply(data, string, callback) {
        ...
        if (data.indexOf("sftp>") > -1 || (this.activeCmd == "bye" && data.indexOf("bye") > -1)) {
            ...
            cbprogress(data);
        }else if (data.indexOf("assword")>-1){
          if (this.options && this.options.password){
            this.socket.write(this.options.password);
            this.socket.write('\r');
          }else{
            return callback("No password supplied");
          }
        }else if (data.indexOf("Are you sure you want to continue connecting")>-1){
            this.socket.write('yes\r');  
        }else if (data.indexOf("Connection closed by remote host")>-1){
            return callback("Error logging in: please check your credentials and try again");
        }
        // If none of these "else-if"'s fire, we've received some data we can't parse in here, 
